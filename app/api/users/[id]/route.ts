    ...
    if (!user) {
      return NextResponse.json(
        { 
          success: false,
          error: "User not found",
          message: "The user you're trying to delete no longer exists."
        },
        { status: 404 }
      );
    }
    ...
    if (bookingsCount > 0) {
      return NextResponse.json(
        { 
          ...
          message: "This user has existing bookings and cannot be deleted. Please deactivate the user instead.",
          bookingsCount
        },
        { status: 400 }
      );
